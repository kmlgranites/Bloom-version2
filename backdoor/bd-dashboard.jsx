...
const D_LOGO = { Stripe:"stripe.com", Vercel:"vercel.com", Ramp:"ramp.com", Notion:"notion.so", Rippling:"rippling.com", Brex:"brex.com", Deel:"deel.com", Amgen:"amgen.com", Wonder:"wonder.com", "Fidelity Investments":"fidelity.com", "OPS Consulting":"opsconsulting.com" };
    ...
    <div style={{display:"flex", flexDirection:"column", gap:20}}>
      <div style={{position:"relative", background:dT.ink, color:"#fff", borderRadius:16, padding:"22px 26px",
        ...
        <div style={{position:"relative"}}>
          ...
          <div style={{fontFamily:dFD, fontWeight:700, fontSize:19, marginBottom:2}}>Free · $0</div>
          <div style={{fontSize:12.5, opacity:.7, fontWeight:600}}>5 applications / month · upgrade any time for unlimited</div>
        </div>
      </div>

      <div style={{display:"flex", gap:16, flexWrap:"wrap"}}>
        <PlanCard name="Free" price="$0" per="forever" blurb="Try Bloom on a handful of roles."
          features={["5 applications / month","Manual apply or Full Auto Apply","Résumé auto-fill","Email support"]} current/>
...
function SettingsView({ st, set, onBack, title, initialSub }) {
...
function Dashboard({ st, set, onSignOut }) {
  ...
  const [pendingSub, setPendingSub] = React.useState("billing");
  ...
  const rows = tab==="All" ? [...rows0].sort((a,b)=>pri(a.status)-pri(b.status)) : rows0;
    ...
    <div style={{height:"100vh", display:"flex", background:"#FFFCF6", fontFamily:dFB, color:dT.ink, overflow:"hidden"}}>
      ...
      <div style={{width:188, flexShrink:0, borderRight:`1px solid ${dT.hairline}`, background:"#fff",
        ...
        <div style={{marginTop:"auto", display:"flex", flexDirection:"column", gap:10, paddingTop:16, paddingBottom:20, borderTop:`1px solid ${dT.hairline}`}}>
          <button style={{width:"100%", padding:"11px 0", borderRadius:999, background:dT.ink, color:"#fff", border:"none",
          ...
          <div style={{background:dT.ink, borderRadius:16, padding:"14px 14px 12px", display:"flex", flexDirection:"column", gap:10}}>
            <div style={{display:"flex", alignItems:"center", gap:8}}>
              ...
              <span style={{fontSize:12.5, fontWeight:700, color:"#fff", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap"}}>Vinodh</span>
            </div>
            <div>
              <div style={{fontSize:11, color:"rgba(255,255,255,.65)", fontWeight:600, marginBottom:5, display:"flex",
                justifyContent:"space-between"}}>
                <span>Free applications used</span><span>{used}/{D_FREE_LEFT}</span>
              </div>
              <div style={{height:5, borderRadius:999, background:"rgba(255,255,255,.18)", overflow:"hidden"}}>
                <div style={{height:"100%", width:`${used/D_FREE_LEFT*100}%`, background:"#5AEBEB", borderRadius:999}}/>
              </div>
            </div>
            <button onClick={goPlans} style={{width:"100%", padding:"9px 0", borderRadius:999, background:"#fff", color:dT.ink,
      ...
      <div style={{flex:1, minWidth:0, minHeight:0, display:"flex", flexDirection:"column"}}>
        <div style={{padding:"0 32px", height:68, flexShrink:0, display:"flex", alignItems:"center", justifyContent:"space-between",
          ...
          <div style={{display:"flex", alignItems:"center", gap:12}}>
            <span title="Applications left on the free plan" style={{display:"inline-flex", alignItems:"center", gap:7, padding:"6px 12px", borderRadius:999,
              background:"#F4F6F6", border:`1px solid ${dT.hairline}`, fontSize:12.5, fontWeight:700, color:dT.ink, whiteSpace:"nowrap"}}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#C9871F" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11.017 2.814a1 1 0 0 1 1.966 0l1.051 5.558a2 2 0 0 0 1.594 1.594l5.558 1.051a1 1 0 0 1 0 1.966l-5.558 1.051a2 2 0 0 0-1.594 1.594l-1.051 5.558a1 1 0 0 1-1.966 0l-1.051-5.558a2 2 0 0 0-1.594-1.594l-5.558-1.051a1 1 0 0 1 0-1.966l5.558-1.051a2 2 0 0 0 1.594-1.594z"/><path d="M20 2v4"/><path d="M22 4h-4"/><circle cx="4" cy="20" r="2"/></svg>
              <span>{`${left} application${left===1?"":"s"} left`}</span>
            </span>
            <button onClick={goPlans} style={{padding:"7px 15px", borderRadius:999, background:dT.ink, color:"#fff", border:"none",
              fontFamily:dFB, fontSize:12.5, fontWeight:700, cursor:"pointer", whiteSpace:"nowrap"}}>Upgrade</button>
            <span style={{width:1, height:18, background:dT.hairline}}/>
        ...
        ) : (view==="settings" || view==="profile") ? <SettingsView st={st} set={set} title={view==="profile" ? "Profile" : "Settings"} initialSub={pendingSub} onBack={()=>setView("pipeline")}/> : (
          ...
          <div style={{flexShrink:0, border:`1px solid ${dT.hairline}`, borderRadius:14, background:"#fff", overflow:"hidden"}}>
            ...
            <div>
              ...
              {rows.length === 0 ? (
                ...
                return (
                  <div key={j.co} onClick={e=>{ if(e.target.closest("button")) return; j._match ? setMatchPanel(j._match) : openReview(j); }} style={{display:"grid", gridTemplateColumns:"1.3fr 1.4fr 1fr minmax(140px,1fr)", gap:12,
                    ...
                    <div style={{color:dT.muted, fontWeight:600, whiteSpace:"nowrap", overflow:"hidden"}}>{j._match ? (
                      isAuto ? (
                        <span style={{display:"inline-flex", alignItems:"center", gap:6, fontSize:12.5, fontWeight:600, color:dT.ink}}>
                          <span style={{width:6, height:6, borderRadius:"50%", background: paused ? "#F2B84B" : "#22A565"}}/>{paused ? "Queued · paused" : left<=0 ? "Waiting · free limit reached" : `Bloom applies in ~${j._rank+1}h`}</span>
                      ) : (
                    ...
                    ) : j.status==="Submitting" ? <LiveTicker steps={D_INFLIGHT_STEPS}/> : j.status==="Preparing" ? "Tailoring résumé…" : j.status==="Needs you" ? "Waiting on you" : j.applied}</div>
                  </div>
                );
              })}
            </div>
