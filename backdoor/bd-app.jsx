...
const INITIAL = {
  role:"Software Engineer", engKinds:["Frontend"], level:"Senior", types:["Fulltime"], setting:"Remote",
          ...
          <div style={{maxWidth: c.bare ? 1034 : 640, margin:"0 auto", padding:"0 36px", display:"flex", flexDirection:"column", gap:14}}>
            {c.bare ? (
              <div className="bd-framed" style={{background:"#fff", border:`1px solid ${aT.hairline}`, borderRadius:24}}>
                ...
                {step===1 && <window.Card1 st={st} set={set} locked={false} onNext={advance} toast={toast}/>}
                {step===2 && <window.ContactCard st={st} set={set} locked={false} onNext={advance} toast={toast}/>}
                {step===3 && <window.WorkCard st={st} set={set} locked={false} onNext={advance} toast={toast}/>}
                {step===4 && <window.ChecklistCard st={st} set={set} locked={false} onNext={advance} toast={toast}/>}
                {step===5 && <window.ModeCard st={st} set={set} locked={false} onNext={()=>setDone(true)} toast={toast}/>}
              </div>
              ...
              return (
                <React.Fragment key={cd.id}>
                  ...
                  {idx===1 && <window.Card1 {...props}/>}
                  {idx===2 && <window.ContactCard {...props}/>}
                  {idx===3 && <window.WorkCard {...props}/>}
                  {idx===4 && <window.ChecklistCard {...props}/>}
                  {idx===5 && <window.ModeCard st={st} set={set} locked={locked} onNext={()=>setDone(true)} toast={toast}/>}
                </React.Fragment>
              );
            })}
          </div>
