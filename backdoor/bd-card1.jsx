...
function Card1({ st, set, locked, onNext }) {
  ...
  if (locked) return (
    <div style={{display:"flex", flexWrap:"wrap", gap:8, padding:"14px 16px", background:"#F9FAFB", borderRadius:16}}>
      {[st.role, st.level, st.engKinds.join(" · "), st.types.join(" · "), st.setting].filter(Boolean).map(x=>(
    ...
    <div style={{display:"grid", gridTemplateColumns:"40% 60%", background:"#fff",
      ...
      <div style={{padding:"44px 40px 48px", display:"flex", flexDirection:"column", gap:24}}>
        ...
        <div style={{display:"grid", gridTemplateColumns:"minmax(0,1fr)", gap:20}}>
          <div>
            <QLabel>LEVEL</QLabel>
            <QSegment value={st.level} onChange={v=>set({level:v})} options={Q_LEVELS}/>
          </div>
          <div>
            <QLabel>WORK SETTING</QLabel>
            <QSegment value={st.setting} onChange={v=>set({setting:v})} options={Q_SETTING}/>
          </div>
        </div>
