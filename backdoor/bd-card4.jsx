    ...
    <div style={{display:"grid", gridTemplateColumns:"40% 60%", background:"#fff",
      ...
      <div style={{background:"#F9FAFB", borderRadius:"0 0 0 24px", padding:"44px 32px 48px",
        ...
        <div>
          <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10}}>
            <span style={{fontSize:11, fontWeight:800, color:kT.muted, letterSpacing:".07em"}}><span style={{fontWeight:700}}>RESUME</span></span>
          ...
          <div style={{fontSize:13, fontWeight:600, color:"#4B5A5E", lineHeight:1.55}}>
            <span style={{fontWeight:500}}>Parsed. While you finish setup, we're matching jobs for you.</span>
          </div>
        </div>
        <div>
          <div style={{fontSize:13.5, fontWeight:800, color:kT.ink, marginBottom:8}}>Why we ask</div>
          <div style={{fontSize:13, fontWeight:500, color:"#4B5A5E", lineHeight:1.55}}>
      ...
      <div style={{padding:"44px 40px 48px", display:"flex", flexDirection:"column", gap:22}}>
        ...
        <div style={{border:`1px solid ${kT.hairline}`, borderRadius:12, overflow:"visible"}}>
          ...
          <div style={{padding:"15px 18px", borderBottom:`1px solid ${kT.hairline}`}}>
            <div style={{fontSize:14, fontWeight:600, color:kT.ink, lineHeight:1.35, marginBottom:11}}>What is your availability / notice period?</div>
            <div role="radiogroup" aria-label="Availability" style={{display:"flex", flexWrap:"wrap", gap:8}}>
              {["Immediately","in 1 week","in 2 weeks","in 1 month","in 2 months"].map(o=>{
                const on = (q.notice || "Immediately") === o;
                return (
                  <button key={o} type="button" role="radio" aria-checked={on} onClick={()=>upd({notice:o})}
                    style={{display:"flex", alignItems:"center", gap:7, padding:"8px 14px 8px 11px", borderRadius:999, cursor:"pointer",
                      fontFamily:kFB, fontSize:13, fontWeight:600, whiteSpace:"nowrap",
                      background: on ? kT.ink : "#fff", color: on ? "#fff" : kT.ink,
                      border:`1.5px solid ${on ? kT.ink : kT.hairline}`}}>
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={{opacity: on ? 1 : .55}}><circle cx="12" cy="12" r="10"/><path d="m8 12 3 3 5-6"/></svg>
                    {o}
                  </button>
                );
              })}
