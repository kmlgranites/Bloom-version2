...
function ApplySettingsPanel({ st, set, goProfile }) {
  ...
  const [s, setS] = React.useState({
    ...
    quality:"Strong", cap:10, askEssay:true, askLong:false, coverText:"",
  ...
  return (
    <div style={{maxWidth:760, margin:"0 auto", background:"#fff", border:`1px solid ${aT.hairline}`, borderRadius:16, padding:"8px 32px 12px"}}>
      ...
      <ASection n="01" title="How Bloom applies">
        ...
        <AField label="Cover letter" hint="Paste your letter. Bloom sends it with applications that ask for one.">
          <textarea value={s.coverText} onChange={e=>u({coverText:e.target.value})} rows={7}
        ...
        <AField label="Work location">
          <div style={{display:"flex", flexDirection:"column", gap:10}}>
            <ASwitchRow label="Remote jobs" on={s.remote} onToggle={()=>u({remote:!s.remote})}>
              ...
              <label style={{display:"flex", alignItems:"flex-start", gap:9, fontSize:13, fontWeight:600, color:aT.ink, cursor:"pointer", lineHeight:1.4}}>
                <input type="checkbox" checked={s.flexZone} onChange={()=>u({flexZone:!s.flexZone})} style={{width:16, height:16, accentColor:aT.ink, margin:"1px 0 0", flexShrink:0}}/>
                <span>Also include remote jobs that don't mention a time zone
                  <span style={{display:"block", fontSize:12, fontWeight:500, color:aT.muted, marginTop:2}}>Most remote posts don't list one. Turning this off hides them.</span></span>
              </label>
            </ASwitchRow>
