...
function NChannel({ id, title, desc, on, onClick, disabled }) {
  return (
    <button type="button" onClick={onClick} disabled={disabled}
      style={{display:"flex", alignItems:"flex-start", gap:11, padding:"14px 15px", borderRadius:10, textAlign:"left",
        border:`1.5px solid ${on ? nT.ink : nT.hairline}`, background: on ? "#FFFCF6" : "#fff",
      ...
      <span style={{minWidth:0}}>
        ...
        <span style={{display:"block", fontSize:12.5, fontWeight:600, color:nT.muted, lineHeight:1.45, marginTop:3}}>{desc}</span>
      </span>
    </button>
  );
}
  ...
  return (
    <div style={{display:"grid", gridTemplateColumns:"40% 60%", background:"#fff",
      ...
      <div style={{padding:"44px 40px 48px", display:"flex", flexDirection:"column", gap:24}}>
        ...
        <div>
          ...
          <div style={{display:"grid", gap:9}}>
            <NChannel on={(st.channel || "whatsapp") === "whatsapp"} disabled={st.verified}
              ...
              title="WhatsApp"
              desc="Best if you already use it — threaded updates you can reply to."/>
            <NChannel on={st.channel === "imessage"} disabled={st.verified}
              onClick={()=>set({channel:"imessage", verified:false, otpSent:false})}
              title="iMessage / SMS"
              desc="No WhatsApp? Updates arrive as a normal text on this number."/>
          </div>
          <div style={{fontSize:11.5, color:nT.muted, fontWeight:600, marginTop:8}}>
        ...
        {/* LinkedIn */}
        <div>
          <NLabel>LINKEDIN URL <span style={{fontWeight:600, color:nT.muted, textTransform:"none", letterSpacing:0}}>(optional)</span></NLabel>
          <input value={st.linkedin || ""} onChange={e=>set({linkedin:e.target.value})}
            placeholder="linkedin.com/in/yourname"
            style={{width:"100%", boxSizing:"border-box", padding:"13px 14px", borderRadius:12,
              border:`1.5px solid ${nT.hairline}`, outline:"none", fontFamily:nFB, fontSize:14.5, fontWeight:600, color:nT.ink}}/>
        </div>
